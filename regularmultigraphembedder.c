...
typedef int GRAPH[MAX_ORDER+1][DEGREE+2];
...
struct EMB_MULG {
	...
	struct EMB_EDGE *edgeIncidention[MAX_ORDER][DEGREE];
...
struct EMB_MULG *mulg;
{
	...
	for (j=0; j< DEGREE; j++){
		fprintf(stderr,"|");
		for (i=0; i< mulg->order; i++){
			fprintf(stderr, "%2d|", mulg->edgeIncidention[i][j]->index);
		}
		fprintf(stderr,"\n");
	}
...
int count_faces(struct EMB_MULG *emb_mulg){
	...
	int counter = 0;
	for(i=0; i<emb_mulg->order; i++){
		struct EMB_EDGE *start = &(emb_mulg->edges[i*DEGREE]);
		/*(*start).marker = 0;*/
		edge = start;
...
void create_multigraph_2(int a[], struct EMB_MULG *mulg){
	...
	for(i = 0; i < DEGREE; i++){
		...
		((mulg->edges)+ DEGREE + i)->index = i;
		
		((mulg->edges)+i)->next = ((mulg->edges)+((i+1)%DEGREE));
		((mulg->edges)+((i+1)%DEGREE))->previous = ((mulg->edges)+i);
		((mulg->edges)+i)->inverse = ((mulg->edges) + DEGREE + i);
		
		((mulg->edges)+ DEGREE + i)->inverse = ((mulg->edges)+i);
	}
	
	for(i = 0; i < DEGREE; i++){
		((mulg->edges)+ DEGREE + *(a+i))->next = ((mulg->edges)+ DEGREE + *(a+(i+1)%DEGREE));
		((mulg->edges)+ DEGREE + *(a+i))->previous = ((mulg->edges)+ DEGREE + *(a+(i+DEGREE-1)%DEGREE));
	}
}
